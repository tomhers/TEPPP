...
map<int, double> jones(string fname, double **coordinates, bool has_coords, int length, int num_proj)
{
    ...
    for (int z = 0; z < num_proj; z++) {
        ...
        for (int i = 0; i < init_crossings; i++) {
            if (are_collinear(coords[crossings[i][0]], coords[crossings[i][1]], coords[crossings[i][2]], coords[crossings[i][3]])) continue;
            if (compute_one(coords[crossings[i][0]], coords[crossings[i][1]], coords[crossings[i][2]], coords[crossings[i][3]]) > 0) writhe++;
            else writhe--;
        }
        ...
        for (int j = start_chunk; j <= end_chunk; j+=step) {
            ...
            for (int k = 0; k < chain_length - j; k++) {
                ...
                for (int z = 0; z < knot_scan.size(); z++) {
                    if (knot_scan[z] == 1) {
                        outfile << "Possible " << knots[z] << " found in chain " << i << " with chunk length " << j << " starting at atom " << k+(i*chain_length) << "\n";
                    }
                }
